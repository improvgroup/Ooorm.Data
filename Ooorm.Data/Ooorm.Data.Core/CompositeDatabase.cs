...
namespace Ooorm.Data.Core
{
    public class CompositeDatabase : IDatabase
    {
        ...
        {
            if (backTypes.Contains(typeof(T)))
                return back.Read<T>(predicate);
            else
                return front.Read<T>(predicate);
        }

        public IAsyncEnumerable<T> Read<T, TParam>(Expression<Func<T, TParam, bool>> predicate, TParam param) where T : IDbItem
        {
            if (backTypes.Contains(typeof(T)))
                return back.Read(predicate, param);
            else
                return front.Read(predicate, param);
        }
        ...
        {
            if (backTypes.Contains(type))
                return back.Read(type);
            else
                return front.Read(type);
        }
