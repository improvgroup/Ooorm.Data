...
namespace Ooorm.Data.Core.Sqlite
{
    ...
    {        
        ...
        public async IAsyncEnumerable<T> Read<T>() where T : IDbItem
            => Repos<T>().Read();

        public async Task<T> Read<T>(int id) where T : IDbItem
            => await Repos<T>().Read(id);

        public async IAsyncEnumerable<T> Read<T>(Expression<Func<T, bool>> predicate) where T : IDbItem
            => Repos<T>().Read(predicate);

        public async IAsyncEnumerable<T> Read<T, TParam>(Expression<Func<T, TParam, bool>> predicate, TParam param) where T : IDbItem
            => Repos<T>().Read(predicate, param);

        public async IAsyncEnumerable<object> Read(Type type)
